...
export default function DetalheProdutoPage({ params }: { params: { id: string } }) {
	...
	const loadProduct = async () => {
		...
		// List linked services: we can't easily list service products by product,
		// but wait! The API doesn't have a direct /products/{id}/services endpoint.
		// Wait! How can we know which services consume this product?
		// We can fetch all services and then load `/services/{id}/products` for each active service!
		// This is extremely robust and is perfect for client side matching.
		// Let's load all services:
		...
		if (resSrv.data) {
			...
			for (const s of svcs) {
				const resLinks = await http.get<EnrichedServiceProduct[]>(`/services/${s.id}/products`)
				if (resLinks.data) {
					...
					matched.forEach((m) => {
						links.push({
							...
							product_name: s.name, // Display service name instead!
							quantity: m.quantity,
							unit: m.unit,
						})
					})
