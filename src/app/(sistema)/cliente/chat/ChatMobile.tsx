...
export default function ChatMobile() {
	...
	return (
		<div className="flex h-[calc(100vh-96px)] overflow-hidden bg-slate-50/50 rounded-2xl border border-slate-200 shadow-sm relative w-full">
				...
				<div className="p-4 border-b border-slate-100 space-y-3">
					<div className="flex items-center justify-between">
						...
						<Button
							...
							className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl h-8 w-8 p-0 flex items-center justify-center shadow-sm"
							title="Nova Conversa"
						>
						...
						filteredChats.map((chat) => {
							...
							return (
								<div
									...
									className="flex items-center gap-3 p-4 cursor-pointer hover:bg-slate-50/80 active:bg-slate-100 transition select-none"
								>
									...
									<div className="h-10 w-10 bg-slate-100 rounded-full overflow-hidden flex items-center justify-center text-slate-500 relative flex-shrink-0 border border-slate-200">
										...
										{chat.customer_id && (
											<span className="absolute bottom-0 right-0 w-3 h-3 bg-emerald-500 rounded-full border border-white flex items-center justify-center">
			...
			{/* CONVERSA ATIVA (VISÍVEL APENAS QUANDO HÁ CHAT SELECIONADO) */}
			<div className={`w-full bg-slate-50/20 flex flex-col h-full relative ${activeChat ? "flex" : "hidden"}`}>
				{activeChat && (
					<>
						{/* HEADER DA CONVERSA */}
						<div className="px-4 py-3 border-b border-slate-200 bg-white flex items-center justify-between z-10 shadow-sm">
							<div className="flex items-center gap-2 min-w-0">
								<button 
									...
									className="p-1 hover:bg-slate-100 rounded-lg transition mr-1"
								>
									<ArrowLeft className="h-5 w-5 text-slate-600" />
								</button>
								<div className="h-9 w-9 bg-slate-100 rounded-full overflow-hidden flex items-center justify-center text-slate-500 border border-slate-200 flex-shrink-0">
							...
							{activeChat.customer_id && (
								<span className="bg-emerald-50 text-emerald-700 border border-emerald-200 text-[9px] font-extrabold px-2 py-0.5 rounded-full flex-shrink-0">
									Agenda
								</span>
							)}
						</div>
						...
						{!activeChat.customer_id && (
							<div className="bg-amber-50 border-b border-amber-200 px-4 py-2 flex items-center justify-between animate-in slide-in-from-top duration-300">
								<span className="text-[10px] text-amber-800 font-semibold truncate mr-2">
									⚠️ Contato fora da Agenda do CRM.
								</span>
								<Button
									onClick={() => handleOpenAddContact(activeChat)}
									className="bg-amber-600 hover:bg-amber-700 text-white font-extrabold h-6 text-[9px] rounded-lg px-2 shadow-sm"
								>
									Salvar
								</Button>
							</div>
							...
							{loadingMessages ? (
								...
								messages.map((msg) => {
									...
									return (
										...
										>
											<div
												className={`max-w-[85%] rounded-2xl px-3.5 py-2 shadow-sm text-xs font-semibold ${
													isOutbound
											...
											>
												...
												<span
													className={`text-[8px] block text-right mt-1 font-normal ${
						...
						<div className="p-3 border-t border-slate-200 bg-white">
							<form onSubmit={handleSendMessage} className="flex gap-2">
								...
								<Button
									...
									className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl px-3 flex items-center justify-center h-9 font-bold"
								>
									{sending ? (
			...
			{showAddContactModal && (
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 backdrop-blur-sm p-4">
					<form 
						...
						className="bg-white rounded-2xl border border-slate-100 shadow-2xl p-5 w-full max-w-sm max-h-[90vh] overflow-y-auto space-y-4"
					>
						<div className="flex items-center justify-between">
							<h3 className="font-bold text-slate-800 text-xs flex items-center gap-1.5">
								<UserPlus className="h-4 w-4 text-indigo-600" />
								Salvar na Agenda (CRM)
							</h3>
							<button 
								type="button"
						...
						<div className="flex gap-2 justify-end pt-1">
							<Button
								...
								variant="outline"
								onClick={() => setShowAddContactModal(false)}
								className="rounded-xl h-9 text-xs font-bold border-slate-200 text-slate-600 px-3"
							>
								Cancelar
							</Button>
							<Button
								...
								className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl h-9 text-xs font-bold px-3 flex items-center gap-1"
							>
								{savingContact ? (
				...
				<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 backdrop-blur-sm p-4">
					...
					>
						...
						<div className="grid grid-cols-2 gap-1 p-1 bg-slate-100 rounded-xl">
							<button
								...
								className={`py-1 text-[10px] font-bold rounded-lg transition ${
									newChatMode === "select" ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500"
								}`}
							>
								Agenda (CRM)
							</button>
							<button
								...
								className={`py-1 text-[10px] font-bold rounded-lg transition ${
									newChatMode === "create" ? "bg-white text-indigo-700 shadow-sm" : "text-slate-500"
								}`}
							>
								Novo Número
							</button>
						...
						<div className="space-y-3 pt-1">
							{newChatMode === "select" ? (
								<div className="space-y-1">
									<label className="text-[9px] font-bold text-slate-500 uppercase">Buscar Cliente</label>
									<div className="relative">
									...
									<div className="border border-slate-100 rounded-xl max-h-[130px] overflow-y-auto mt-2 divide-y divide-slate-50 bg-slate-50/20">
										{loadingCrm ? (
											<div className="p-3 text-center text-slate-400 flex justify-center items-center gap-1">
												<Loader2 className="h-3.5 w-3.5 animate-spin text-indigo-600" />
												<span className="text-[10px] font-medium">Buscando...</span>
											</div>
											...
											crmCustomers.map((cust) => {
												...
												return (
													<div
														...
														className={`p-2 text-xs cursor-pointer transition flex items-center justify-between select-none ${
							...
							<div className="space-y-1">
								<label className="text-[9px] font-bold text-slate-500 uppercase">Mensagem Inicial</label>
								<Input
